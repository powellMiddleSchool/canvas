...
$(document).ready(function(){
	if (!hcpssHasRun) {
		...
		/* Add container for extra links to nav */
		$('#menu').after('<ul id="extra-nav"></ul>');

		/* Add in extra nav items  */
		if(typeof(ENV) !== 'undefined' &&
			(ENV.current_user_roles.indexOf('admin') != -1 || ENV.current_user_roles.indexOf('teacher') != -1)
		){
			var url = "https://hcpss.instructure.com/courses/378/pages/canvas-communities";
			var label = "Communities";
			$('#menu').append("<li class='menu-item' id='community_menu_item' > <a class='menu-item-no-drop' href='" + url + "' target='_blank'>"+label+"</a> </li>");
			$('#community_menu_item').click(function() { 
				trackLinkClick('headerLinkClick', 'Communities'); 
			});

			var url = "https://staff.hcpss.org/";
			var label = "Staff Hub";
			$('#extra-nav').append("<li class='menu-item' > <a id='link-staff-hub' href='" + url + "' class='menu-item-no-drop' target='_blank'>"+label+"</a> </li>");

			/* Email Link */
			/* var url = "https://mail.hcpss.org";
			var label = "Email";
			$('#extra-nav').append("<li class='menu-item' > <a id='link-email' href='" + url + "' class='menu-item-no-drop' target='_blank'>"+label+"</a> </li>");
			*/
		...
		/* New Assignment page: remove "Add Group" option from Assignment Group dropdown */
		/* Done on an interval several times because I think this might be dynamically added? */
		/* // Commented out 10/8/15 per Joe's request
		$("#assignment_group_id option[value=new]").remove();
		var intervalTimes2 = 0;
		...
		*/
		...
		/* Remove "Set Group Weights" option from gradebook */
		/* Done on an interval several times because I think this might be dynamically added? */
		/* // Commented out 10/8/15 per Joe's request
		if ($('body.gradebook2').length > 0) {
			...
			var intervalID3 = setInterval(function() {
				$('.gradebook_dropdown li').each(function() {
					if ($(this).children('a').text() == "Set Group Weights") {
						$(this).remove();
					}
				});
				if (++intervalTimes3 === 8) {
		...
		*/
		...
		/* Remove 'report a problem' and 'submit a feature' from help dialog - only for students/observers */
		...
		*/
		...
		// Accordion - collapse by default if it has the accordion_collapse class
